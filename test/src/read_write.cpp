...
int main() {
  ...
  "read_simple_tree"_test = [&] {
    expect(t_simple.traverse<ReadOperation>(10)                         == ID_NOT_FOUND);    // Must return an error, as leafnode does not exist
    expect(t_simple.traverse<ReadOperation>(11)                         == ID_NOT_FOUND);    // Must return an error, as leafnode does not exist
    expect(t_simple.traverse<ReadOperation>(0,1,2)                      == ID_NOT_FOUND);    // Must return an error, as leafnode does not exist
    expect(t_simple.traverse<ReadOperation>(-4)                         == ID_NOT_FOUND);    // Must return an error, as leafnode does not exist
    expect(t_simple.traverse<ReadOperation>(0)                          == NO_ERROR);        // Must return no error, as leafnode does exist
    expect(ReadOperation::getValue<int>()                               == 5_i);             // Variable must equal 5, as 5 was written to leafnode
    expect(t_simple.traverse<ReadOperation>(1)                          == NO_ERROR);        // Must return no error, as leafnode does exist
    expect(ReadOperation::getValue<double>()                            == 5.5_d);           // Variable must equal 5.5, as 5.5 was written to leafnode
    expect(t_simple.traverse<ReadOperation>(2)                          == NO_ERROR);        // Must return no error, as leafnode does exist
    expect(ReadOperation::getValue<float>()                             == -4.5_f);          // Variable must equal -4.5, as -4.5 was written to leafnode
    expect(t_simple.traverse<ReadOperation>(3)                          == NO_ERROR);        // Must return no error, as leafnode does exist
  ...
  "write_simple_tree"_test = [&] {
    ...
    expect(t_simple.traverse<WriteOperation>(1)                                   == NO_ERROR);       // Must return no error, as leafnode does exist
    expect(t_simple.traverse<ReadOperation>(1)                                    == NO_ERROR);       // Must return no error, as leafnode does exist
    ...
    expect(t_simple.traverse<WriteOperation>(2)                                   == NO_ERROR);       // Must return no error, as leafnode does exist
    expect(t_simple.traverse<ReadOperation>(2)                                    == NO_ERROR);       // Must return no error, as leafnode does exist
    ...
    expect(t_simple.traverse<WriteOperation>(3)                                   == NO_ERROR);       // Must return no error, as leafnode does exist
    expect(t_simple.traverse<ReadOperation>(3)                                    == NO_ERROR);       // Must return no error, as leafnode does exist
    ...
    expect(t_simple.traverse<WriteOperation>(4)                                   == NO_ERROR);       // Must return no error, as leafnode does exist
    expect(t_simple.traverse<ReadOperation>(4)                                    == NO_ERROR);       // Must return no error, as leafnode does exist
    expect(ReadOperation::getValue<std::chrono::seconds>().count()                == 8_i);
    WriteOperation::setValue<int(*)(int,int)>(&sumMock);
    expect(t_simple.traverse<WriteOperation>(6)                                   == NO_ERROR);
    expect(t_simple.traverse<ReadOperation>(6)                                    == NO_ERROR);
    expect(ReadOperation::getValue<int(*)(int,int)>()(3,4)                        == 7_i);
    WriteOperation::setValue<int(*)(int,int)>(nullptr);
    expect(t_simple.traverse<WriteOperation>(7)                                   == NO_ERROR);
    expect(t_simple.traverse<ReadOperation>(7)                                    == NO_ERROR);
    expect(ReadOperation::getValue<int(*)(int,int)>()                             == nullptr);
    Mock m(4,2,&sumMock);
    WriteOperation::setValue<std::function<int()>>(std::bind(
        &Mock::sum,&m,1,2));
    expect(t_simple.traverse<WriteOperation>(9)                                   == NO_ERROR);
    expect(t_simple.traverse<ReadOperation>(9)                                    == NO_ERROR);
  ...
  "read_exist_asym_tree"_test = [&] {
    expect(t_asym.traverse<ReadOperation>(0,10)                     == ID_NOT_FOUND);       // Must return an error, as leafnode does not exist
    expect(t_asym.traverse<ReadOperation>(0,11)                     == ID_NOT_FOUND);       // Must return an error, as leafnode does not exist
    expect(t_asym.traverse<ReadOperation>(1,14)                     == ID_NOT_FOUND);       // Must return an error, as leafnode does not exist
    expect(t_asym.traverse<ReadOperation>(0,0)                      == NO_ERROR);           // Must return no error, as leafnode does exist
    expect(ReadOperation::getValue<int>()                           == 5_i);                // Variable must equal 5, as 5 was written to leafnode
    expect(t_asym.traverse<ReadOperation>(0,1)                      == NO_ERROR);           // Must return no error, as leafnode does exist
    expect(ReadOperation::getValue<double>()                        == 5.5_d);              // Variable must equal 5.5, as 5.5 was written to leafnode
    expect(t_asym.traverse<ReadOperation>(0,2)                      == NO_ERROR);           // Must return no error, as leafnode does exist
    expect(ReadOperation::getValue<float>()                         == -4.5_f);             // Variable must equal -4.5, as -4.5 was written to leafnode
    expect(t_asym.traverse<ReadOperation>(0,3)                      == NO_ERROR);           // Must return no error, as leafnode does exist
    expect(ReadOperation::getValue<std::array<char,255>>()[0]       == 'h');                // Variable must equal "hello", as "hello" was written to leafnode
    expect(ReadOperation::getValue<std::array<char,255>>()[1]       == 'e');
    expect(ReadOperation::getValue<std::array<char,255>>()[2]       == 'l');
    expect(t_asym.traverse<ReadOperation>(1)                        == NO_ERROR);           // Must return no error, as leafnode does exist
    expect(ReadOperation::getValue<double>()                        == 2.5_d);              // Variable must equal 2.5, as 2.5 was written to leafnode
    expect(t_asym.traverse<ReadOperation>(0,4)                      == NO_ERROR);
    expect(ReadOperation::getValue<std::chrono::seconds>().count()  == 5_i);
    expect(t_asym.traverse<ReadOperation>(0,5)                      == NO_ERROR);
    expect(ReadOperation::getValue<Mock>().f                        == 2.3_f);
    expect(t_asym.traverse<ReadOperation>(2)                        == NO_ERROR);
    expect(ReadOperation::getValue<int(*)(int,int)>()               == nullptr);
    expect(t_asym.traverse<ReadOperation>(3)                        == NO_ERROR);
    expect(ReadOperation::getValue<std::function<int()>>()          == nullptr);
  ...
  "write_asym_tree"_test = [&] {
    ...
    expect(t_asym.traverse<WriteOperation>(0,0)                                    == NO_ERROR);       // Must return no error, as leafnode does exist
    expect(t_asym.traverse<ReadOperation>(0,0)                                     == NO_ERROR);       // Must return no error, as leafnode does exist
    ...
    expect(t_asym.traverse<WriteOperation>(0,1)                                    == NO_ERROR);       // Must return no error, as leafnode does exist
    expect(t_asym.traverse<ReadOperation>(0,1)                                     == NO_ERROR);       // Must return no error, as leafnode does exist
    ...
    expect(t_asym.traverse<WriteOperation>(0,2)                                    == NO_ERROR);       // Must return no error, as leafnode does exist
    expect(t_asym.traverse<ReadOperation>(0,2)                                     == NO_ERROR);       // Must return no error, as leafnode does exist
    expect(ReadOperation::getValue<float>()                                        == -4.3_f);         // Variable must equal -4.3, as -4.3 was written to leafnode
    WriteOperation::setValue<std::array<char,255>>(std::array<char,255>{"test"});
    expect(t_asym.traverse<WriteOperation>(0,3)                                    == NO_ERROR);       // Must return no error, as leafnode does exist
    expect(t_asym.traverse<ReadOperation>(0,3)                                     == NO_ERROR);       // Must return no error, as leafnode does exist
    expect(ReadOperation::getValue<std::array<char,255>>()[0]                      == 't');            // Variable must equal 'test', as 'test' was written to leafnode
    expect(ReadOperation::getValue<std::array<char,255>>()[1]                      == 'e');
    WriteOperation::setValue<double>(-10.3);
    expect(t_asym.traverse<WriteOperation>(1)                                      == NO_ERROR);       // Must return no error, as leafnode does exist
    expect(t_asym.traverse<ReadOperation>(1)                                       == NO_ERROR);       // Must return no error, as leafnode does exist
    expect(ReadOperation::getValue<double>()                                       == -10.3_d);        // Variable must equal -10.3, as -10.3 was written to leafnode
    WriteOperation::setValue<int(*)(int,int)>(&sumMock);
    expect(t_asym.traverse<WriteOperation>(2)                                      == NO_ERROR);
    expect(t_asym.traverse<ReadOperation>(2)                                       == NO_ERROR);
    expect(ReadOperation::getValue<int(*)(int,int)>()(4,4)                         == 8_i);
    Mock m(4,2,&sumMock);
    WriteOperation::setValue<std::function<int()>>(std::bind(
        &Mock::sum,&m,8,2));
    expect(t_asym.traverse<WriteOperation>(3)                                      == NO_ERROR);
    expect(t_asym.traverse<ReadOperation>(3)                                       == NO_ERROR);
